...
-------------------------------------------------------------------
BEGIN EXECUTE IMMEDIATE 'DROP TABLE stg_crime_register_good PURGE'; 
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE stg_crime_register_bad PURGE'; 
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE stg_officer_good PURGE'; 
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE stg_officer_bad PURGE'; 
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE stg_location_good PURGE'; 
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE stg_location_bad PURGE'; 
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE stg_crime_type_good PURGE'; 
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE stg_crime_type_bad PURGE'; 
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/

-------------------------------------------------------------------
-- 2. Create GOOD/BAD tables
-------------------------------------------------------------------
CREATE TABLE stg_crime_register_good AS 
SELECT * FROM stg_crime_register WHERE 1=0;
/

CREATE TABLE stg_crime_register_bad AS
SELECT sr.*, CAST(NULL AS VARCHAR2(400)) AS error_reason
FROM stg_crime_register sr WHERE 1=0;
/

CREATE TABLE stg_officer_good AS 
SELECT * FROM stg_officer WHERE 1=0;
/

CREATE TABLE stg_officer_bad AS
SELECT so.*, CAST(NULL AS VARCHAR2(400)) AS error_reason
FROM stg_officer so WHERE 1=0;
/

CREATE TABLE stg_location_good AS 
SELECT * FROM stg_location WHERE 1=0;
/

CREATE TABLE stg_location_bad AS
SELECT sl.*, CAST(NULL AS VARCHAR2(400)) AS error_reason
FROM stg_location sl WHERE 1=0;
/

CREATE TABLE stg_crime_type_good AS 
SELECT * FROM stg_crime_type WHERE 1=0;
/

CREATE TABLE stg_crime_type_bad AS
SELECT sct.*, CAST(NULL AS VARCHAR2(400)) AS error_reason
FROM stg_crime_type sct WHERE 1=0;
/

-------------------------------------------------------------------
-- 3. QUALITY ETL PACKAGE
-------------------------------------------------------------------
...
CREATE OR REPLACE PACKAGE BODY quality_etl_pkg AS
  ...
  BEGIN
    ...
    FROM stg_crime_register sr
    WHERE NOT (
      sr.reported_date IS NOT NULL AND
      sr.location_id IS NOT NULL AND
      sr.officer_id IS NOT NULL AND
      sr.crime_type_id IS NOT NULL AND
      (sr.is_closed IS NULL OR sr.is_closed IN (0,1))
    );
    ...
    etl_log_pkg.end_process(v_process_id,'SUCCESS',v_rows_good+v_rows_bad,v_rows_good,0,v_rows_bad,0,NULL);
  ...
  BEGIN
    ...
    FROM stg_officer so
    WHERE NOT (
      so.officer_name IS NOT NULL AND
      so.officer_rank IS NOT NULL AND
      so.department IS NOT NULL
    );
  ...
  BEGIN
    ...
    FROM stg_location sl
    WHERE NOT (
      sl.station_name IS NOT NULL AND
      sl.region_id IS NOT NULL AND
      sl.city IS NOT NULL
    );
  ...
  BEGIN
    ...
    FROM stg_crime_type sct
    WHERE NOT (
      sct.crime_type IS NOT NULL AND
      sct.severity_level IS NOT NULL
    );
...
CREATE OR REPLACE PROCEDURE load_dim_date IS
BEGIN
    ...
    SELECT DISTINCT
        TO_NUMBER(TO_CHAR(t.reported_date, 'YYYYMMDD')) AS date_id,
        t.reported_date,
        EXTRACT(YEAR FROM t.reported_date),
        EXTRACT(MONTH FROM t.reported_date),
        EXTRACT(DAY FROM t.reported_date),
        'TRF_CRIME_REGISTER'
    FROM trf_crime_register t
    WHERE t.reported_date IS NOT NULL
      AND NOT EXISTS (
            SELECT 1
            FROM dim_date d
            WHERE d.full_date = t.reported_date
      );
